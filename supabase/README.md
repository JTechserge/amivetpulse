...
| `push-server` | Envoie des notifications Web Push (VAPID) aux abonnés. Appelée en fire-and-forget par le front, qui doit présenter le JWT d'un compte. | `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_CONTACT_EMAIL` |
