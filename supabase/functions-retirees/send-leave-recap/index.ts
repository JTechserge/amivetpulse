...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const slots = dataRows[0] && dataRows[0].data || {};
    const pending = [];
    ...
    for (const key of Object.keys(slots)){
      ...
      if (m && slots[key] === 'pending') {
        ...
        pending.push({
          ...
          personId,
          slot: slot,
          label: slots[`${iso}_${personId}_${slot}_label`] || ''
        });
      }
    }
    const now = new Date();
    ...
    const text = [
      ...
      ...overtimeEntries.length ? overtimeEntries.map(([p, h])=>`- ${p} : ${h}h`) : [
    ...
    const overtimeRowsHtml = overtimeEntries.length ? overtimeEntries.map(([p, h])=>`<tr>
        ...
        </tr>`).join('') : `<tr><td style="padding:6px 0;font-size:13px;color:${COLORS.textMuted};">Aucune heure supplémentaire enregistrée sur cette période.</td></tr>`;
