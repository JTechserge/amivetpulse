import {
  PEOPLE, ASV_PEOPLE, SLOTS, SLOT_LABELS,
  MONTH_NAMES, WEEKDAY_NAMES, WEEKLY_MAX_HOURS,
  SUPABASE_FUNCTIONS_URL, personOf,
} from './config.js';
import {
  escapeHTML, hexToRgba,
  fmtISO, daysInMonth, isoWeekday, isSunday, holidayName,
  formatHHMM, signedHHMM, roundTo15min, formatFR, getWeekMondayDate,
} from './utils.js';
...
import {
  isASVPerson, isWithinNextTwoWeeks, isPersonWorkingDay,
  getSlotState, setSlotState, getSlotLabel, setSlotLabel,
  getDayComment, setDayComment, cycleState,
  getLeaveDecision, setLeaveDecision, getLeaveDecisionComment,
...
function setupCalendar({
  snapshotBeforeChange, saveData, switchSubPage, canEditSlot, undoLastAction, getCurrentView,
}) {
...
function openAbsenceLabelPopover(cell, forceAbsent){
  ...
  box.querySelector('#popover-sick').onclick = ()=>{
    ...
    _saveData(); if(viewKey) renderCalendarView(viewKey); close();
  };
  ...
  document.addEventListener('dblclick', (e)=>{
    ...
    if(dayCol && _getCurrentView() === 'asv' && store.subNavState.asv !== 'week'){
      if(!dayCol.classList.contains('cal-wg-day-we')){
        ...
        if(iso){ store.weekNavState.mondayISO = fmtISO(getWeekMondayDate(new Date(iso+'T00:00:00'))); _switchSubPage('asv','week'); }
      }
      return;
    }
    // Vue mensuelle : cellule individuelle avec data-date
    const monthCell = e.target.closest('.cal-cell[data-date]');
    if(monthCell && _getCurrentView() === 'asv' && (store.subNavState.asv === 'calendar' || store.subNavState.asv === 'forecast')){
...
export {
  setupCalendar, cellRenderInfo, cellAriaLabel, updateCellDOM, updateHalfDOM,
  buildCalendarGrid, buildWeekGrid, buildPersonRowCells, buildOvertimeRowCells,
  buildLegendColors, buildLegend, renderCalendarView, changeMonth, goToToday,
  openDaySidebar, openAbsenceLabelPopover, openAbsenceRangePopover, initCalendarInteractions,
  cycleCellAndSave, buildCalendarToolbar, buildSignaturePanelHtml, buildPrintSignatureStatusHtml,
  adminRequestSignature, requestSignatureEmail, calViewKeyOfEventTarget, clearMonth, openClearMonthModal,
};
