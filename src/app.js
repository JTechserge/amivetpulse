...
function renderWeekViewASV(){
  ...
  function buildLunchOtCell(d){
    const iso=fmtISO(d);
    const cg=cellGrey(d);
    if(isSunday(d)) return `<td class="week-ot-cell" style="background:#f8fafc;"></td>`;
  ...
  const weekTotalH=days.reduce((s,d)=>{if(isSunday(d)||!isDayPresent(d))return s;const iso=fmtISO(d);return s+getDayNominal(iso,pid)+getDayAllOtH(iso,pid)-getDayDeficitH(iso,pid);},0);
