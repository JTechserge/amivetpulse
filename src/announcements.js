...
// ----------------------------------------------------------------
export function annonceViewerId() {
  return store.currentUser?.person_id || store.currentUser?.id || '';
}
...
export async function loadAnnouncements() {
  ...
  try {
    ...
    const [annRes, readRes] = await Promise.all([
      fetch(
        `${SUPABASE_URL}announcements?select=*&or=(expires_at.is.null,expires_at.gt.${encodeURIComponent(today)})&order=pinned.desc,created_at.desc`,
        { headers: supabaseHeaders() }
      ),
      fetch(
        `${SUPABASE_URL}announcement_reads?person_id=eq.${encodeURIComponent(annonceViewerId())}&select=announcement_id`,
        { headers: supabaseHeaders() }
      ),
    ]);
    ...
    console.warn('Annonces inaccessibles.', e);
  }
}
...
export async function loadArchivedAnnouncements() {
  try {
    ...
    const res = await fetch(
      `${SUPABASE_URL}announcements?select=*&expires_at=lte.${encodeURIComponent(today)}&order=created_at.desc`,
      { headers: supabaseHeaders() }
    );
...
export function renderAnnounces() {
  ...
  container.querySelectorAll('.ann-filter-pill').forEach((btn) => {
    btn.onclick = () => {
      store.announcementsCache.filter = btn.dataset.cat;
      renderAnnounces();
    };
  });
  ...
  container.querySelectorAll('.ann-edit-btn').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      openAnnouncementModal(btn.dataset.annId);
    };
  });
  ...
  container.querySelector('#ann-archives').addEventListener('toggle', async function () {
    ...
    if (!archived.length) {
      archList.innerHTML = '<p style="font-size:13px;color:var(--color-muted);">Aucune archive.</p>';
      return;
    }
    // eslint-disable-next-line no-unsanitized/property
...
export function openAnnouncementModal(annId) {
  ...
  const catOptions = Object.entries(ANNONCE_CATEGORIES)
    .map(([k, c]) => {
      const sel = k === initCat;
  ...
  titleInput.oninput = () => {
    countEl.textContent = `(${titleInput.value.length}/80)`;
  };
  ...
  box.querySelector('#ann-save-btn').onclick = async () => {
    ...
    if (!title || !content) {
      showToast('Titre et contenu requis', '⚠️');
      return;
    }
    ...
    try {
      ...
      if (existing) {
        ...
        if (pinned) store.announcementsCache.list = [ann, ...store.announcementsCache.list];
          ...
          store.announcementsCache.list = [ann, ...store.announcementsCache.list].sort(
      ...
      if (!existing) {
        const targetUsers =
          target_roles === 'vet' ? ['david', 'stephane'] : target_roles === 'asv' ? ASV_PEOPLE.map((p) => p.id) : [];
