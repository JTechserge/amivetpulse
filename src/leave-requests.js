    ...
    years.forEach((year) => {
      for (let month = 0; month < 12; month++) {
        ...
        for (let day = 1; day <= nbDays; day++) {
          ...
          SLOTS.forEach((slot) => {
            if (getSlotState(iso, person.id, slot) !== 'absent') {
              current = null;
              return;
            }
            const label = getSlotLabel(iso, person.id, slot);
            if (isReposLabel(label) || isSickOrAccidentLabel(label)) {
              current = null;
              return;
            }
...
export function sortLeaveGroups(groups) {
  ...
  return groups
    .slice()
    .sort((a, b) => order[a.status] - order[b.status] || a.slots[0].iso.localeCompare(b.slots[0].iso));
}
...
export function renderLeaveRequestsPage() {
  ...
  const leaveRows = groups
    ...
    `
          : g.comment
            ? `<p class="text-muted" style="font-size:12.5px;margin-top:6px;">💬 ${escapeHTML(g.comment)}</p>`
            : '';
      return `
  ...
  container.querySelectorAll('[data-reject-confirm]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (!comment) {
        showToast('Un commentaire est nécessaire pour refuser', '⚠️');
        return;
      }
...
export function renderGroupConges(group, containerId) {
  ...
  (async () => {
    let adjustments = [];
    try {
      const res = await fetch(`${SUPABASE_URL}cp_adjustments?year=eq.${cpYear}&select=*`, {
        headers: supabaseHeaders(),
      });
      if (res.ok) adjustments = await res.json();
    } catch (e) {
      console.warn('cp_adjustments inaccessibles', e);
    }

    const adjByPerson = {};
    adjustments.forEach((a) => {
      adjByPerson[a.person_id] = a;
    });
    ...
    function soldeColor(b) {
      return b >= 10 ? '#16A34A' : b >= 5 ? '#CA8A04' : '#DC2626';
    }
    function swatch(c) {
    ...
    const yearBtns = [cpYear - 1, cpYear, cpYear + 1]
      .map(
        (y) =>
    ...
    container.querySelectorAll('.cp-year-btn').forEach((btn) => {
      btn.onclick = () => {
        renderGroupConges._year[group] = parseInt(btn.dataset.year, 10);
      ...
      container.querySelectorAll('.cp-adjust-btn').forEach((btn) => {
        btn.onclick = () =>
          openCPAdjustModal(
            btn.dataset.pid,
            cpYear,
            parseFloat(btn.dataset.carried) || 0,
            parseFloat(btn.dataset.extra) || 0,
            btn.dataset.note || '',
            group,
            containerId
          );
...
export function getAbsenteeismRate(personId, year, month) {
  // module absentéisme supprimé de l'UI
...
export function renderDashboardAbsences() {
  // stub — module supprimé
