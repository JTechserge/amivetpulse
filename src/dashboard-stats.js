...
export function computeYearStats(year) {
    ...
    for (let day = 1; day <= nbDays; day++) {
      ...
      all.forEach((p) => {
        ...
        SLOTS.forEach((slot) => {
          ...
          if (state === 'present') {
            stats[p.id].halfDaysByMonth[month]++;
            presentAny = true;
          } else if (state === 'absent') {
            stats[p.id].absentHalfDaysByMonth[month]++;
          }
        });
        ...
        if (isASVPerson(p.id)) {
          ...
          if (isPresent3)
            stats[p.id].overtimeHoursByMonth[month] +=
              getDayAllOtH(iso, p.id) - getDayDeficitH(iso, p.id) + getOvertimeHours(iso, p.id);
        } else {
  ...
  all.forEach((p) => {
    ...
    let busiest = 0;
    s.halfDaysByMonth.forEach((v, i) => {
      if (v > s.halfDaysByMonth[busiest]) busiest = i;
    });
...
export function buildVetAnnualDifferential(year) {
  const stats = computeYearStats(year);
  const todayMonth = new Date().getFullYear() === year ? new Date().getMonth() : 11;
  const rows = PEOPLE.map((p) => {
    const tf = p.timeFraction ?? 1.0;
    const targetH = Math.round(ANNUAL_FULLTIME_HOURS * tf);
    // Heures réelles = demi-journées × 3,5h + heures sup (tous mois)
    const actualH = roundTo15min(stats[p.id].totalHalfDays * HALFDAY_HOURS + stats[p.id].totalOvertimeHours);
    const diff = roundTo15min(actualH - targetH);
    const pct = targetH > 0 ? Math.min(100, Math.round((actualH / targetH) * 100)) : 0;

    // Tableau mensuel : heures réelles cumulées vs objectif cumulé prorata
    let monthRows = '';
    let cumActual = 0;
    for (let m = 0; m <= todayMonth; m++) {
      const mActual = roundTo15min(stats[p.id].halfDaysByMonth[m] * HALFDAY_HOURS + stats[p.id].overtimeHoursByMonth[m]);
      cumActual = roundTo15min(cumActual + mActual);
      const cumTarget = roundTo15min(targetH * ((m + 1) / 12));
      const mDiff = roundTo15min(cumActual - cumTarget);
      const diffCls = mDiff > 0 ? 'color:#16A34A' : mDiff < 0 ? 'color:#DC2626' : 'color:var(--color-text-muted)';
      const sign = mDiff >= 0 ? '+' : '';
      monthRows += `<tr><td>${MONTH_SHORT[m]}</td><td style="font-variant-numeric:tabular-nums">${formatHHMM(cumActual)}</td><td style="font-variant-numeric:tabular-nums">${formatHHMM(cumTarget)}</td><td style="font-variant-numeric:tabular-nums;${diffCls}">${sign}${formatHHMM(Math.abs(mDiff))}${mDiff < 0 ? ' ↓' : mDiff > 0 ? ' ↑' : ''}</td></tr>`;
    }

    const diffColor = diff > 0 ? '#16A34A' : diff < 0 ? '#DC2626' : 'var(--color-text-muted)';
    const diffSign = diff >= 0 ? '+' : '';
    return `
      <div class="card" style="border-top:3px solid ${p.color};margin-bottom:16px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
          <div style="width:36px;height:36px;border-radius:50%;background:${p.color};display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:15px;">${p.initial}</div>
          <div>
            <div style="font-size:15px;font-weight:700;">${p.short}</div>
            <div style="font-size:11px;color:var(--color-text-muted);">Objectif ${targetH}h — ${year}</div>
          </div>
          <div style="margin-left:auto;text-align:right;">
            <div style="font-size:20px;font-weight:800;color:${diffColor}">${diffSign}${formatHHMM(Math.abs(diff))}</div>
            <div style="font-size:11px;color:var(--color-text-muted);">vs objectif 1607h</div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
          ...
          <span style="font-size:12px;font-weight:700;color:${p.color};min-width:40px;">${formatHHMM(actualH)}</span>
          <span style="font-size:11px;color:var(--color-text-muted);">/ ${targetH}h (${pct}%)</span>
        </div>
        ${monthRows ? `
        <details style="font-size:12px;">
          <summary style="cursor:pointer;font-size:11px;color:var(--color-text-muted);margin-bottom:6px;">Détail mensuel cumulé</summary>
          <div style="overflow-x:auto;">
          <table style="width:100%;border-collapse:collapse;font-size:11px;">
            <thead><tr style="color:var(--color-text-muted);font-weight:700;">
              <th style="text-align:left;padding:2px 6px;">Mois</th>
              <th style="text-align:right;padding:2px 6px;">Réel cumulé</th>
              <th style="text-align:right;padding:2px 6px;">Objectif cumulé</th>
              <th style="text-align:right;padding:2px 6px;">Différentiel</th>
            </tr></thead>
  ...
  return `
    <div style="margin-bottom:24px;">
      <h3 style="font-size:16px;margin-bottom:4px;">Différentiel annuel — Objectif 1607h</h3>
      <p class="text-muted" style="font-size:11.5px;margin-bottom:14px;">Heures réellement travaillées vs objectif légal de modulation (art. L3122-4 CT). Positif = avance, négatif = retard.</p>
      ${rows}
    </div>`;
}
...
export function computeOvertimeStats(year) {
  ...
  for (let month = 0; month < 12; month++) {
    ...
    for (let day = 1; day <= nbDays; day++) {
      ...
      ASV_PEOPLE.forEach((p) => {
        stats[p.id][month] += getOvertimeHours(iso, p.id);
      });
    }
  }
  return stats;
...
export function computeASVWorkedHours(personId, year, month = null) {
  ...
  for (const m of months) {
    ...
    for (let day = 1; day <= nb; day++) {
      ...
      const isPresent =
        getSlotState(iso, personId, 'M') === 'present' || getSlotState(iso, personId, 'AM') === 'present';
      if (isPresent)
        total += getDayNominal(iso, personId) + getDayAllOtH(iso, personId) - getDayDeficitH(iso, personId);
...
export function buildHoursControlCard(year) {
  ...
  function barPct(worked, quota) {
    return Math.min(100, quota > 0 ? Math.round((worked / quota) * 100) : 0);
  }
  function alertIcon(cls) {
    return cls === 'over' ? '🔴' : cls === 'warn' ? '🟡' : '🟢';
  }
...
export function buildDashWeeklyMonthCard(year, month) {
  ...
  let rows = '',
    monthOT = ASV_PEOPLE.map(() => 0);
  for (const mon of weeks) {
    const endW = new Date(mon);
    endW.setDate(endW.getDate() + 5);
  ...
  const totalCols = ASV_PEOPLE.map((p, i) => {
    ...
    return `<td style="text-align:right;padding:7px 10px;font-weight:700;">
      ${
        ot !== 0
          ? `<span style="color:${color}">${signedHHMM(ot)}</span>
      <span class="text-muted" style="font-size:11px;"> (${dayEq >= 0 ? '+' : ''}${formatNum(dayEq)}j)</span>`
...
export function buildASVWeeklyCapCard() {
  const mon = store.weekNavState.mondayISO
    ? new Date(store.weekNavState.mondayISO + 'T00:00:00')
    : getWeekMondayDate(today);
  const endW = new Date(mon);
  endW.setDate(endW.getDate() + 5);
  ...
  const rows = asv
    .map((p) => {
      ...
      const diffStr =
        Math.abs(diff) < 0.6
          ? 'équilibre ✅'
          : `${diff > 0 ? '+' : ''}${diff} vs moy.${Math.abs(diff) > 2 ? ' ⚠️' : ''}`;
