...
function openColorPickerModal() {
  ...
  box.querySelector('#color-save').onclick = () => {
    ...
    const newColors = {};
    all.forEach((p) => {
      newColors[p.id] = box.querySelector(`#color-input-${p.id}`).value;
    });
...
function openManageUsersModal() {
  ...
  fetch(`${SUPABASE_FUNCTIONS_URL}manage-users`, {
    ...
    .then((data) => {
      ...
      const localOnlyVets = PEOPLE.filter(
        (p) =>
          !linkedPersonIds.has(p.id) &&
          !users.some(
            (u) =>
              (u.display_name || '').toLowerCase().includes(p.short.toLowerCase()) ||
              (u.display_name || '').toLowerCase().includes(p.name.toLowerCase())
          )
      );
      ...
      const rows = users
        .map((u) => {
          ...
          const swatch = cp
            ? `<span style="display:inline-block;width:11px;height:11px;border-radius:50%;background:${cp.color};margin-right:6px;vertical-align:middle;flex-shrink:0;"></span>`
            : '';
          return `<tr>
      ...
      const localASVRows = localOnlyPeople
        .map(
          (p) => `<tr>
    ...
    `;
      ...
      box.querySelectorAll('.invite-tf-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
          box.querySelectorAll('.invite-tf-btn').forEach((b) => {
      ...
      box.querySelector('#invite-btn').onclick = async () => {
        ...
        if (!name || !email) {
          errEl.textContent = 'Nom et email requis.';
          ...
          if (inviteData.user_id) {
            ...
            if (role === 'vet') {
              ...
              const existing = ASV_PEOPLE.find(
                (p) => p.name.trim().toLowerCase() === name.trim().toLowerCase() && !linkedPersonIds.has(p.id)
              );
              const asvPerson = existing || addASVPerson(name, inviteLastName);
              if (existing && inviteLastName) {
                existing.lastName = inviteLastName;
                saveASVRoster();
              }
...
function wireTimeFractionUI(box, _personId) {
  ...
  box.querySelectorAll('.edit-tf-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      box.querySelectorAll('.edit-tf-btn').forEach((b) => {
        b.classList.remove('active');
        b.style.background = 'var(--color-card)';
        b.style.color = '';
        b.style.borderColor = 'var(--color-border)';
...
function openEditUserModal(userId, users, onBack) {
  ...
  box.querySelector('#edit-save').onclick = async () => {
    ...
    if (colorInput && personId) {
      ...
      if (reason) {
        if (colorErrEl) {
          colorErrEl.textContent = reason;
          colorErrEl.style.display = 'block';
        }
        return;
      }
    }
    ...
    try {
      const payload = {
        ...
        role,
        can_edit_vet_calendar: canVet,
        can_edit_all_asv: canAsv,
      };
      ...
      if (role === 'asv' && personId) {
        ...
        if (p) {
          ...
          if (tfResult) {
            p.timeFraction = tfResult.fraction;
            p.workingDays = tfResult.workingDays;
          }
          saveASVRoster();
        }
      }
  ...
  const sendAccessEmail = async (type) => {
    ...
    try {
      ...
      if (res.status === 401) {
        await _authRefreshSession();
        res = await fetch(`${SUPABASE_FUNCTIONS_URL}manage-users`, {
          method: 'POST',
          headers: supabaseHeaders({ 'Content-Type': 'application/json' }),
          body: JSON.stringify({ action: 'send_access_email', user_id: userId, type }),
        });
...
async function updateCalendarSyncPreferences(personId, syncPresence, syncAbsences, color) {
  const res = await fetch(`${SUPABASE_URL}rpc/update_calendar_sync_preferences`, {
    ...
    body: JSON.stringify({
      ...
      p_sync_presence: syncPresence,
      p_sync_absences: syncAbsences,
      p_color: color,
    }),
  });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
...
function initSettingsMenu() {
  ...
  if (_canAccessSettings()) {
    document.getElementById('action-colors').addEventListener(
      'click',
      () => {
        menu.classList.remove('open');
    ...
    document.getElementById('action-calendar-sync').addEventListener(
      'click',
      () => {
        menu.classList.remove('open');
    ...
    document.getElementById('action-export').addEventListener(
      'click',
      () => {
        ...
        a.href = url;
        a.download = `amivet_planning_${fmtISO(new Date())}.json`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        URL.revokeObjectURL(url);
        menu.classList.remove('open');
        showToast('Export JSON téléchargé', '⬇️');
      },
      { signal }
    );
    const fileInput = document.getElementById('import-file-input');
    document.getElementById('action-import').addEventListener(
      'click',
      () => {
        menu.classList.remove('open');
        fileInput.click();
      ...
      (e) => {
        ...
        reader.onload = () => {
          try {
            ...
            showToast('Fichier JSON invalide', '⚠️');
          }
          fileInput.value = '';
        };
    ...
    document.getElementById('action-manage-users').addEventListener(
      'click',
      () => {
        menu.classList.remove('open');
        openManageUsersModal();
  ...
  if (store.currentUser?.role === 'admin') {
    document.getElementById('action-toggle-view').addEventListener(
      'click',
      () => {
        menu.classList.remove('open');
  ...
  document.getElementById('action-help').addEventListener(
    'click',
    () => {
      menu.classList.remove('open');
      openHelpModal();
  ...
  document.getElementById('action-notifications').addEventListener(
    'click',
    () => {
      menu.classList.remove('open');
  ...
  document.getElementById('action-change-password').addEventListener(
    'click',
    () => {
      menu.classList.remove('open');
