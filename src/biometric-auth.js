...
export function biometricLabel() {
  ...
  return 'Connexion par clé d\'accès';
}

export async function isBiometricAvailable() {
  try {
    return !!(
      window.PublicKeyCredential && (await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable())
    );
  } catch {
    return false;
  }
...
export function clearBiometric() {
  [KEY_ENROLLED, KEY_REFRESH, KEY_EMAIL, KEY_CRED_ID, 'bio_cred'].forEach((k) => localStorage.removeItem(k));
}
